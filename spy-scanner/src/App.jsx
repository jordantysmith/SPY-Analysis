...
export default function App() {
  ...
  const runScan = async () => {
    ...
    const steps = ["Fetching S&P futures…","Checking SPY pre-market…","Reading VIX…","Scanning economic calendar…","Checking Fed schedule…","Reviewing earnings…","Scanning headlines…","Assessing technicals…","Building trade plan…"];
    let si = 0;
    const stTimer = setInterval(() => { setMsg(steps[si % steps.length]); si++; }, 2500);
    ...
    const prompt = `You are a pre-market trading analyst. Today is ${today}, ET time ${etNow}.

Search the web for current data: S&P 500 futures price and % change, SPY pre-market price and % change, VIX level, today's US economic calendar (especially 8:30 AM releases), Fed speakers today, major overnight earnings, key overnight news headlines, SPY technical trend vs recent levels.

After searching, respond with ONLY a JSON object. No markdown, no backticks, no explanation. Just the JSON starting with { and ending with }:
...
Trade rules: $1000 size, medium-high risk. BULLISH=CALL target +75% stop -45%. BEARISH=PUT same. Mixed=NO TRADE. VIX>25 tighten stop 10%. VIX>30 recommend $500 size.`;
    ...
    try {
      ...
      for (let turn = 0; turn < 8; turn++) {
        ...
        messages.push({ role: "user", content: tools.map(b => ({ type: "tool_result", tool_use_id: b.id, content: "Results retrieved." })) });
      }
      ...
      const p = JSON.parse(raw.substring(js, je+1));
